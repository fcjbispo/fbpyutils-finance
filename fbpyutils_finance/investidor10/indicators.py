...
def get_fii_indicators(
    ...
    try:
        ...
        if len(info) == 4:
            ...
            print(
                f"Warning: Invalid info tuple length ({len(info)}). Expected 3 or 4. Skipping."
            )
            return None
        ...
        if isinstance(capture_date, str):
            try:
                ...
                print(
                    f"Warning: Invalid capture_date string format for {fii_ticker}. Expected YYYY-MM-DD. Using None."
                )
        ...
        price_date_info = cotacao_body.find(
            "i"
        )  # Assuming date info is in an <i> tag's attribute
        ...
        if price_date_info and "data-content" in price_date_info.attrs:
            ...
            try:
                fii_indicator_data["DATA_COTAÇÃO"] = datetime.strptime(
                    date_str, "%d/%m/%Y"
                ).date()
            except ValueError:
                fii_indicator_data["DATA_COTAÇÃO"] = None
                print(
                    f"Warning: Could not parse price date '{date_str}' for {fii_ticker}."
                )
        else:
            fii_indicator_data["DATA_COTAÇÃO"] = None
        ...
        if indicators_table:
            for cell in indicators_table.findAll("div", attrs={"class": "cell"}):
                ...
                if key_tag and value_tag:
                    ...
                    value_str = tag_to_str(
                        value_tag.find("span")
                    )  # Get text from inner span
                    fii_indicator_data[key] = value_str  # Store raw string first
                else:
                    # Log if cell structure is unexpected
                    print(
                        f"Warning: Unexpected cell structure in indicators table for {fii_ticker}."
                    )

        else:
            print(
                f"Warning: Could not find indicators table ('div#table-indicators') for {fii_ticker}."
            )
        ...
        fii_indicator_data["NUMERO DE COTISTAS"] = (
            int(num_cotistas) if num_cotistas is not None else None
        )

        cotas_emitidas = any_to_number(cotas_str)
        fii_indicator_data["COTAS EMITIDAS"] = (
            int(cotas_emitidas) if cotas_emitidas is not None else None
        )
        ...
        if val_pat_str:
            ...
            if len(parts) >= 3:
                fii_indicator_data["VALOR PATRIMONIAL"] = any_to_number(
                    parts[1]
                )  # The numeric part
                fii_indicator_data["VALOR PATRIMONIAL UNIT"] = parts[
                    2
                ]  # The unit (Bi/Mi)
            else:
                fii_indicator_data["VALOR PATRIMONIAL"] = any_to_number(
        ...
        fii_indicator_data["VACÂNCIA"] = any_to_number(
            vacancia_str
        )  # Already handles %
        ...
        print(
            f"Error fetching indicators for {fii_ticker if 'fii_ticker' in locals() else 'Unknown'}: {e}"
        )
        return None
    except AttributeError as e:
        print(
            f"Error parsing HTML for {fii_ticker if 'fii_ticker' in locals() else 'Unknown'}: {e}"
        )
        return None
    except Exception as e:
        # Catch any other unexpected errors during processing
        print(
            f"Unexpected error processing indicators for {fii_ticker if 'fii_ticker' in locals() else 'Unknown'}: {e}"
        )
        return None
