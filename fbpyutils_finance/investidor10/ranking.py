...
def get_fii_dy_ranking_data(fii_dy_details_url: str) -> pd.DataFrame:
    ...
    if not dy_ranking_table:
        ...
        if not dy_ranking_table:
            raise AttributeError(
                f"Could not find ranking table with id 'rankings' or 'rankigns' on {fii_dy_details_url}"
            )

    dy_rows = dy_ranking_table.findAll("tr")
    if len(dy_rows) <= 1:  # Check if there's more than just the header row
        print(
            f"Warning: No data rows found in the ranking table on {fii_dy_details_url}. Returning empty DataFrame."
        )
        return pd.DataFrame(
            columns=[
                "ticker",
    ...
    for dy_row in dy_rows[1:]:
        ...
        if len(cells) < 8:
            print(
                f"Warning: Skipping row due to insufficient cells ({len(cells)} found, expected 8): {dy_row}"
            )
            continue
        ...
        try:
            ...
            (
                ...
                dy_current_cell,
                dy_p_vp_cell,
                dy_daily_liquidity_cell,
                dy_net_worth_cell,
                dy_ytd_var_cell,
                dy_fund_type_cell,
                dy_segment_cell,
            ) = cells[:8]  # Take first 8
            ...
            dy_ytd_var = any_to_number(
                tag_to_str(dy_ytd_var_cell)
            )  # Assumes YTD Var is numeric %
            ...
            dy_daily_liquidity = (
                any_to_number(liquidity_parts[0]) if liquidity_parts else None
            )
            dy_daily_liquidity_unit = (
                liquidity_parts[1] if len(liquidity_parts) > 1 else None
            )

            net_worth_str = tag_to_str(dy_net_worth_cell)
            net_worth_parts = net_worth_str.split(" ")
            dy_net_worth = (
                any_to_number(net_worth_parts[0]) if net_worth_parts else None
            )
            ...
            print(
                f"Warning: Could not parse row in ranking table. Error: {e}. Row: {dy_row}"
            )
    ...
    if not dy_data:
        print(
            f"Warning: No data could be successfully parsed from ranking table on {fii_dy_details_url}. Returning empty DataFrame."
        )
    ...
    ranking_df["reference_date"] = pd.to_datetime(
        ranking_df["reference_date"], errors="coerce"
    ).dt.date
