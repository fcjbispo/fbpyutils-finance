...
def get_ifix_data(ifix_page_url: str) -> pd.DataFrame:
    ...
    try:
        ...
        raise SystemError(
            f"Unexpected error during HTTP request to {ifix_page_url}: {e}"
        )
    ...
    if not ifix_table_body:
        raise AttributeError(
            f"Could not find 'tbody' element on the page: {ifix_page_url}"
        )
    ...
    for row in ifix_table_body.children:
        if isinstance(row, Tag) and row.name == "tr":  # Process only table row tags
            ...
            if ticker_anchor and title_tag and share_cell:
                ...
                if isinstance(
                    details_url, Tag
                ):  # Should not happen with .get() but check anyway
                    details_url = tag_to_str(details_url)
                elif not isinstance(details_url, str):
                    details_url = None  # Set to None if not a string

                if (
                    ticker and title and share is not None
                ):  # Only append if essential data is valid
                    ifix_data.append((ticker, title, share, details_url, CAPTURE_DATE))
                else:
                    print(
                        f"Warning: Skipping row due to missing data (Ticker: {ticker}, Title: {title}, Share: {share_str})"
                    )

    if not ifix_data:
        print(
            f"Warning: No valid IFIX data parsed from {ifix_page_url}. Returning empty DataFrame."
        )
    ...
    ifix_df["reference_date"] = pd.to_datetime(
        ifix_df["reference_date"], errors="coerce"
    ).dt.date
