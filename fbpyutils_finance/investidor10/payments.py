...
def get_fii_all_payment_data(fiis_page_url: str) -> pd.DataFrame:
    ...
    if fiis_page.status_code != 200:
        raise SystemError(
            f"Http Error not 200: {fiis_page.status_code} for URL: {fiis_page_url}"
        )
    ...
    for month_group in list_content.findAll(
        "div", attrs=({"class": "month-group-payment"})
    ):
        ...
        for payment_card in month_group.findAll("div", attrs={"class": "payment-card"}):
            try:
                payment_day_str = tag_to_str(
                    payment_card.find("div", attrs={"class": "payment-day"})
                )
                payment_month_str = tag_to_str(
                    payment_card.find("div", attrs={"class": "text-center"})
                )
            ...
            for payment_row in payment_card.find_all(
                "div", attrs={"class": "row payment-row"}
            ):
                try:
                    ticker_anchor = payment_row.find(
                        "a", attrs={"class": "fii-ticker"}, href=True
                    )
                    if not ticker_anchor:
                        continue  # Skip if no ticker link
                    ...
                    payment_str = tag_to_str(p_tags[0]).split(" ")[
                        -1
                    ]  # Expects format "Provento R$ X.XX"
                    fii_pmt = any_to_number(payment_str)  # Use robust conversion

                    # COM date (Data COM)
                    com_date_str = p_tags[1].text.split(" ")[
                        -1
                    ]  # Expects format "Data COM DD/MM/YYYY"
                    ...
                    print(
                        f"Warning: Could not parse payment row for ticker {fii_ticker if 'fii_ticker' in locals() else 'Unknown'}. Error: {e}"
                    )
    ...
    fii_payment_dates["payment"] = pd.to_numeric(
        fii_payment_dates["payment"], errors="coerce"
    )
    fii_payment_dates["com_date"] = pd.to_datetime(
        fii_payment_dates["com_date"], errors="coerce"
    ).dt.date
    fii_payment_dates["payment_date"] = pd.to_datetime(
        fii_payment_dates["payment_date"], errors="coerce"
    ).dt.date
    fii_payment_dates["reference_date"] = pd.to_datetime(
        fii_payment_dates["reference_date"], errors="coerce"
    ).dt.date

    return fii_payment_dates


def get_fii_payment_data(
    fiis_page_url: str, date_type: Literal["com", "payment"]
) -> pd.DataFrame:
    """
    ...
    if date_type not in ("com", "payment"):
        raise TypeError(
            f"Invalid date_type: '{date_type}'. Must be 'com' or 'payment'."
        )
    ...
    content_div = soup.find(
        "div", attrs={"id": "list-content"}
    )  # Or a more specific container
    if not content_div:
        raise AttributeError(
            "Could not find the main content container ('div' with id 'list-content')."
        )

    fii_payment_rows = content_div.find_all("div", attrs={"class": "row payment-row"})
    if not fii_payment_rows:
        print(
            f"Warning: No payment rows found on {fiis_page_url}. Returning empty DataFrame."
        )
    ...
    # Ensure correct dtypes
    fii_payment_dates_df["payment"] = pd.to_numeric(
        fii_payment_dates_df["payment"], errors="coerce"
    )
    fii_payment_dates_df[f"{date_type}_date"] = pd.to_datetime(
        fii_payment_dates_df[f"{date_type}_date"], errors="coerce"
    ).dt.date
    fii_payment_dates_df["reference_date"] = pd.to_datetime(
        fii_payment_dates_df["reference_date"], errors="coerce"
    ).dt.date
