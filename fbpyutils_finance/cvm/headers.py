...
from .processing import get_expression_and_converters
...
def get_cvm_file_metadata(cvm_file_path: str) -> Tuple[str, str, str, str]:
    ...
    try:
        ...
        try:
            with open(cvm_file_path, 'r', encoding='utf-8') as f:
                header_line = f.readline().strip() # Read first line and remove trailing newline
        except UnicodeDecodeError:
            print(f"Warning: Decoding {cvm_file_path} as utf-8 failed, trying iso-8859-1.")
    ...
    metadata_part = file_name_parts[1].lower() # e.g., inf_cadastral_fi_20231231 or cad_fi
    ...
    # Add more rules here if other sub_kinds exist based on filename patterns
         ...
         print(f"Warning: Unknown sub_kind pattern in filename: {file_name}. Using '{metadata_part.upper()}' as sub_kind.")
...
def get_cvm_updated_headers(
    ...
    try:
        ...
        if cvm_files is None or not isinstance(cvm_files, list):
            ...
            if not cvm_files:
                 print("Warning: Empty cvm_files list provided to get_cvm_updated_headers.")
                 return current_headers_df.to_dict('records') if not current_headers_df.empty else []
            else:
        ...
        required_mapping_cols = ["Kind", "Order", "Target_Field", "Source_Field", "Transformation1", "Transformation2", "Transformation3", "Converter"]
        if not all(col in header_mappings_df.columns for col in required_mapping_cols):
             ...
             if not all(col.lower() in cols_lower for col in required_mapping_cols):
                  raise ValueError(f"Header Mappings DataFrame is missing required columns (case-insensitive check failed): {required_mapping_cols}")
             else:
                  # Normalize column names in the mapping df if needed
                  header_mappings_df.columns = [c.lower() for c in header_mappings_df.columns]


        # Group mappings by 'Kind' directly from the DataFrame
        # Ensure 'Kind' column exists and handle potential case issues
        kind_col = next((col for col in header_mappings_df.columns if col.lower() == 'kind'), None)
        order_col = next((col for col in header_mappings_df.columns if col.lower() == 'order'), None)
        if not kind_col or not order_col:
             raise ValueError("Header Mappings DataFrame must contain 'Kind' and 'Order' columns.")
        ...
        for cvm_file_path in cvm_files:
            try:
                kind, sub_kind, header, header_hash = get_cvm_file_metadata(cvm_file_path)
                ...
                processed_files_count += 1
                 ...
                 print(f"Warning: Skipping file due to metadata extraction error: {cvm_file_path} - {e}")
            except Exception as e:
                print(f"Warning: Unexpected error processing file, skipping: {cvm_file_path} - {e}")
        ...
        existing_mappings_list = current_headers_df.to_dict('records') if not current_headers_df.empty else []
        # Add 'Is_New' flag = False to existing mappings
        hash_col_current = next((col for col in current_headers_df.columns if col.lower() == 'hash'), None) if not current_headers_df.empty else None
        existing_hashes = set(m.get(hash_col_current) for m in existing_mappings_list) if hash_col_current else set()
        ...
        for kind, sub_kind, header, header_hash in if_source_headers:
            if header_hash not in existing_hashes:
                print(f"New header hash detected: {header_hash} for Kind={kind}, SubKind={sub_kind}")
                # Find the appropriate template mapping based on Kind
                if kind not in header_template_map:
                    print(f"Warning: No header mapping template found for Kind='{kind}'. Cannot process hash {header_hash}.")
                    continue
                ...
                t_order_col = next((k for k in template_keys if k.lower() == 'order'), 'Order')
                t_target_col = next((k for k in template_keys if k.lower() == 'target_field'), 'Target_Field')
                t_source_col = next((k for k in template_keys if k.lower() == 'source_field'), 'Source_Field')
                t_trans1_col = next((k for k in template_keys if k.lower() == 'transformation1'), 'Transformation1')
                t_trans2_col = next((k for k in template_keys if k.lower() == 'transformation2'), 'Transformation2')
                t_trans3_col = next((k for k in template_keys if k.lower() == 'transformation3'), 'Transformation3')
                t_conv_col = next((k for k in template_keys if k.lower() == 'converter'), 'Converter')
                ...
                for template_map in header_template:
                    ...
                    found = source_field is not None and source_field in source_fields_in_file
                    ...
                    new_map_entry = {
                        ...
                        'Transformation1': template_map.get(t_trans1_col) if found else None,
                        'Transformation2': template_map.get(t_trans2_col) if found else None,
                        'Transformation3': template_map.get(t_trans3_col) if found else None,
                ...
                for field_in_file in source_fields_in_file:
                    if field_in_file not in mapped_source_fields_for_this_hash:
                        current_order += 1
                        print(f"  -> Found unmapped source field: '{field_in_file}' in hash {header_hash}. Adding with Order={current_order}.")
                        unmapped_entry = {
                            'Kind': kind,
                            'Sub_Kind': sub_kind,
                            'Header': header,
        ...
        consolidated_mappings.sort(key=lambda x: (x.get('Hash', ''), int(x.get('Order', 0))))

        return consolidated_mappings

    except Exception as E:
        info = debug_info(E)
        raise ValueError(f'Failed to get updated CVM headers at step {step}: {E} ({info})')


def check_cvm_headers_changed(cvm_files: List[str], current_headers_df: pd.DataFrame) -> Set[str]:
    """
    ...
    try:
        ...
        if current_headers_df is None or current_headers_df.empty:
             ...
             hash_col_current = next((col for col in current_headers_df.columns if col.lower() == 'hash'), None)
             existing_hashes = set(current_headers_df[hash_col_current]) if hash_col_current else set()
        ...
        for cvm_file_path in cvm_files:
             try:
                 ...
                 print(f"Warning: Skipping file check due to metadata error: {cvm_file_path} - {e}")
             except Exception as e:
                 print(f"Warning: Unexpected error processing file, skipping check: {cvm_file_path} - {e}")
        ...
        info = debug_info(E)
        raise ValueError(f'Failed to check CVM headers changed at step {step}: {E} ({info})')
    ...
    try:
        ...
        if not mappings:
            ...
            if os.path.exists(file_path):
                 print(f"Skipping write to {file_path} as mappings list is empty and file exists.")
                 return False
            else:
                 print(f"Writing empty DataFrame to {file_path} as mappings list is empty and file does not exist.")
                 # Create an empty DataFrame with expected columns
                 expected_cols = ['Hash', 'Kind', 'Sub_Kind', 'Order', 'Target_Field', 'Source_Field',
                                  'Transformation1', 'Transformation2', 'Transformation3', 'Converter', 'Is_New', 'Header']
             ...
             base_cols = ['Hash', 'Kind', 'Sub_Kind', 'Order', 'Target_Field', 'Source_Field',
                          'Transformation1', 'Transformation2', 'Transformation3', 'Converter', 'Is_New']
        ...
        info = debug_info(E)
        raise ValueError(f'Failed to write CVM header mappings to {file_path} at step {step}: {E} ({info})')
