...
def build_target_file_name(
    ...
    original_ext = (
        base_name.split(".")[-1] if "." in base_name else "txt"
    )  # Default extension
...
def write_target_file(
    ...
    try:
        os.makedirs(
            os.path.dirname(target_file), exist_ok=True
...
def read_cvm_history_file(
    ...
    try:
        ...
        if check_header:
            ...
            if changed_hashes:
                raise ValueError(
                    f"Header changed for file {source_file}! Hashes: {changed_hashes}"
                )
        ...
        if not mappings:
            raise ValueError(
                f"No header mappings found for hash {header_hash} in file {source_file}. Headers might have changed."
            )
        ...
        if not expressions:  # Mappings might exist but result in no expressions if all source fields are null
            raise ValueError(
                f"No expressions generated from mappings for hash {header_hash}. Check mappings for file {source_file}."
            )
        if apply_conversions and not data_converters:
            raise ValueError(
                f"No converters found for hash {header_hash}, but apply_converters is True. Check mappings for file {source_file}."
            )
        ...
        if if_data.empty:
            ...
            expected_cols = [
                m["Target_Field"].lower() for m in mappings if m.get("Target_Field")
            ]
            partition_cols = [
                "kind",
                "sub_kind",
        ...
        if_data.columns = [
            c.lower() for c in if_data.columns
        ...
        if apply_conversions:
            ...
            cvm_if_data = apply_converters(
                cvm_if_data.copy(), data_converters
        ...
        if (
            "position_date" in cvm_if_data.columns
            and not cvm_if_data["position_date"].isnull().all()
        ):
            try:
                # Ensure it's datetime before formatting
                pos_date_dt = pd.to_datetime(
                    cvm_if_data["position_date"], errors="coerce"
                )
                ...
                print(
                    f"Warning: Could not compute period info from 'position_date' in {source_file}: {e}"
                )
        elif (
            "start_date" in cvm_if_data.columns
            and not cvm_if_data["start_date"].isnull().all()
        ):
            try:
                start_date_dt = pd.to_datetime(
                    cvm_if_data["start_date"], errors="coerce"
                )
                ...
                print(
                    f"Warning: Could not compute period info from 'start_date' in {source_file}: {e}"
                )
            ...
            try:
                ...
                if len(parts) >= 3 and parts[1].startswith("inf_cadastral_fi_"):
                    ...
                    len(parts) >= 2 and parts[1] == "cad_fi"
                ):  # Current file, use today's date
                ...
                if date_part_str:
                    ...
                    print(
                        f"Warning: Could not extract date from filename for CAD_FI: {file_name}"
                    )

            except Exception as e:
                print(
                    f"Warning: Error computing period info for CAD_FI file {source_file}: {e}"
                )
        ...
        raise ValueError(
            f'Failed processing CVM history file "{source_file}" at step {step}: {E} ({info})'
        )
