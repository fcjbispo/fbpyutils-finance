...
# Import base finance package for constants like _ROOT_DIR, USER_CVM_DIR
...
# Uses FI._ROOT_DIR which should be defined in fbpyutils_finance package
HEADERS_FILE = os.path.join(FI._ROOT_DIR, "cvm", "data", "if_headers_v4.xlsx")
# Path to the Excel file containing the base mapping templates (used to generate HEADERS_FILE).
HEADER_MAPPINGS_FILE = os.path.join(
    FI._ROOT_DIR, "cvm", "data", "if_header_mappings.xlsx"
)
...
try:
    if not os.path.exists(HEADERS_FILE):
        _initialization_error = FileNotFoundError(
            f"CVM Headers File not found at: {HEADERS_FILE}"
        )
    else:
    ...
    if not os.path.exists(HEADER_MAPPINGS_FILE):
        ...
        _initialization_error = FileNotFoundError(
            f"CVM Header Mappings File not found at: {HEADER_MAPPINGS_FILE}"
        )
    else:
...
if _initialization_error:
    print(
        f"FATAL: Failed to initialize fbpyutils_finance.cvm package: {_initialization_error}"
    )
